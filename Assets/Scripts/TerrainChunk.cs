...
public class TerrainChunk {
	...
	public TerrainChunk(Vector2 coord, HeightMapSettings heightMapSettings, MeshSettings meshSettings,  LODInfo[] detailLevels, int colliderLODIdx, Transform parent, Transform viewer, Material material) {
		this.coord = coord;
    ...
    public void Load() {
        ThreadedDataRequestor.RequestData(() => HeightMapGenerator.GenerateHeightMap(meshSettings.numVtxPerLine, meshSettings.numVtxPerLine, heightMapSettings, sampleCentre), OnHeightMapReceived);
    }
		...
		if(!hasSetCollider) {
			float sqrDstFromViewerToEdge = bounds.SqrDistance(viewerPos);
				...
				if(!lodMeshes[colliderLODIdx].hasRequestedMesh) {
					lodMeshes[colliderLODIdx].RequestMesh(heightMap, meshSettings);
				}
			}
